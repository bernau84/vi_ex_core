...
class vi_ex_cell : public class vi_ex_hid
{
private:
    ...
    template <typedef T> std::vector<T> param(std::string name, t_vi_param_flags f){
    ...
    template <typedef T> std::vector<T> setup(std::string name, std::vector<T> &v, u8 action){
...
public:
    ...
    template <typedef T> std::vector<T> def(std::string name){ //defaultni hodnota parametru pokud ocekavame pole

        return param(name, VI_TYPE_P_DEF);
    }

    //vraci 3 hodnoty v poradi nazvu fce pokud je polozka tohoto typu jinak 0
    template <typedef T> std::vector<T> minmaxstep(std::string name){
    ...
    template <typedef T> std::vector<T> menudef(std::string name){
    ...
    std::set<std::string> stringdef(std::string name){
    ...
    template <typedef T> std::vector<T> set(std::string name, std::vector<T> &v){

        return setup(name, v, 3);
    }

    //cteni konfigurace
    template <typedef T> std::vector<T> get(std::string name){
    ...
    std::map<std::string, std::vector<u8>> neighbours(void){
        ...
        if(neighbours.empty()){
            ...
            memset(echo.h.marker, 0, sizeof(mark)); //udelame z toho broadcast nebo unicast podle act_mark
            submit(echo);
        }
        return neighbours;
    }
